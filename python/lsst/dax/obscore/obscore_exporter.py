...
class ObscoreExporter:
    ...
    def to_votable(self, limit: int | None = None) -> astropy.io.votable.tree.VOTableFile:
        ...
        resource.coordinate_systems.append(
            astropy.io.votable.tree.CooSys(system="ICRS", ID=coosys_id, config=votable_config)
        )
        timesys_id = "timesys"
        resource.time_systems.append(
            astropy.io.votable.tree.TimeSys(
                timeorigin="MJD-origin", timescale="UTC", ID=timesys_id, config=votable_config
            )
        )
