...
-- internal
    obsID uuid not null references hsc_schema.Observation (obsID),
...
create index i_obsID on hsc_schema.Plane (obsID)
;

-- tag the clustering index
cluster i_obsID on hsc_schema.Plane
;

create unique index i_planeURI on hsc_schema.Plane(planeURI)
;

create unique index i_publisherID on hsc_schema.Plane(publisherID)
;

create unique index i_creatorID on hsc_schema.Plane(creatorID)
    where creatorID is not null
;

-- join table
create table hsc_schema.ProvenanceInput
(
    outputID uuid not null references hsc_schema.Plane (planeID),
    inputID varchar(512) not null
)
;

create unique index i_output2input on hsc_schema.ProvenanceInput (outputID,
inputID)
;

create unique index i_input2output on hsc_schema.ProvenanceInput (inputID,
outputID)
;
