...
create table hsc_schema.Observation
(
...
create unique index i_observationURI on hsc_schema.Observation (observationURI)
;

create unique index i_observationURI2 on hsc_schema.Observation (collection,
observationID)
;

-- harvesting index
create index i_maxLastModified on hsc_schema.Observation (maxLastModified)
;

-- member join support
create table hsc_schema.ObservationMember
(
    parentID uuid not null references hsc_schema.Observation (obsID),
    memberID varchar(512) not null
)
;

create unique index i_parent2member on hsc_schema.ObservationMember (parentID,
memberID)
;

create unique index i_member2parent on hsc_schema.ObservationMember (memberID,
parentID)
;
